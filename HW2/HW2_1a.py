		...
		for i in range(0,y_tr.shape[0]):
			...
			for a in range(0,w1.shape[0]):
				for b in range(0, w1.shape[1]):
					w1[a][b] = w1[a][b] - 2*lr*np.sum((y_hat[i]-y_tr[i])*sigmoidprime(out)*w2[:,a])*sigmoidprime(hidden[a])*x_tr[i][b]
					b1[a] = b1[a] - 2*lr*np.sum((y_hat[i]-y_tr[i])*sigmoidprime(out)*w2[:,a])*sigmoidprime(hidden[a])
