	...
	for epochs in range(0,noe):
		...
		for a in range(0,w2.shape[0]):
			for b in range(0,w2.shape[1]):
				w2[a][b] = w2[a][b] - lr*np.sum(2*(y_hat[:,a]-y_tr[:,a])*sigmoidprime(y_hat[:,a])*hidden_act[b])
				b2[a] = b2[a] - lr*np.sum(2*(y_hat[:,a]-y_tr[:,a])*sigmoidprime(y_hat[:,a]))

		for a in range(0,w1.shape[0]):
			for b in range(0, w1.shape[1]):
				# print np.sum((y_hat-y_tr)*sigmoidprime(y_hat)*w2[:,a],axis = 0).shape, sigmoidprime(hidden_act[a]), x_tr[:,b].shape
				w1[a][b] = w1[a][b] - 2*lr*np.sum(np.sum((y_hat-y_tr)*sigmoidprime(y_hat)*w2[:,a],axis = 1)*sigmoidprime(hidden_act[a])*x_tr[:,b])
				b1[a] = b1[a] - 2*lr*np.sum(np.sum((y_hat-y_tr)*sigmoidprime(y_hat)*w2[:,a],axis = 1)*sigmoidprime(hidden_act[a]))
			# print i
